...
**Mandatory sandbox execution rule (Codex / sandboxed agents):** Never invoke the draw.io desktop CLI inside a sandbox. Before every `drawio` / `draw.io` command — including `--version`, Mermaid conversion, ELK `--layout`, preview export, and final export — request execution outside the sandbox (Codex: `sandbox_permissions: "require_escalated"`) and include a concrete justification. Use this reason template, adjusting only the command details:

> Allow running `drawio` outside the sandbox. draw.io is an Electron app and may crash or hang inside the sandbox, so this export/convert command needs unsandboxed execution.

This also applies to bundled scripts that shell out to draw.io (`drawiohtml.py`, `drawio2pptx.py`, `svgflow.py`, `buildup.py`, `prdiff.py`, `timelapse.py`). In sandboxed macOS environments the Electron process can crash or produce no output, even for `drawio --version`. If escalated execution is denied or unavailable, treat the CLI as unavailable: do not retry inside the sandbox; use the browser fallback / XML-only outputs or ask the user to run the export outside the sandbox.
...
**Image-reading consent (required).** If the user's request involves a document, directory, archive, page, or attachment that contains images, and completing the request would require reading any of those images, **ask first** whether to read them. Do **not** run OCR, vision, or any other image-reading tool on user-provided images without explicit approval. A direct request such as "read this file/image/figure" is consent for that named image; the user can also approve an entire document. If the user declines, continue using only text/metadata and skip image-based steps. Images produced by this skill's own export pipeline (e.g. the step-5 preview PNG) are not user-provided document images and may be read during self-check without extra consent.

**File access scope (required).** Read user files only when the user explicitly names them as reference material or explicitly designates a directory, repository, or workspace as the input scope. If the user names one file, read only that file; do not scan siblings, ancestors, the current working directory, or the whole repository. If the user designates a directory or workspace, read only within that designated scope. Before reading any user file beyond the named scope, ask. This rule does not block reading this skill's own bundled docs/scripts/built-ins, or its managed state under `~/.drawio-skill/styles/`; those are skill resources, not user reference material.

**Existing `.drawio` reuse consent (required).** Unless the user explicitly asks to use, compare, convert, restyle, explain, or learn from an existing `.drawio` file, do not open or borrow any existing `.drawio` file as a template, style source, layout baseline, or structural example. New diagrams are authored from the user's description and this skill's bundled references, not by silently reusing `.drawio` files found in the working directory, repository, or workspace. When the user does name a `.drawio` file, read only that named file and stay within the file access scope above.

**Color restraint (required).** Outside black, white, and grays, use soft/muted colors and limit the diagram to at most three theme colors unless the user explicitly requests a different palette. A theme color is a hue family, not a single hex value: tints, shades, and low-saturation variants of the same hue count as one theme (for example, `#f0f4ff`, `#dae8fc`, `#6c8ebf`, and `#1a3a5c` are all one blue theme). Reuse theme colors across roles and tiers instead of adding a new theme per node type. A user style preset or an explicit color request overrides this default. For layered architecture, the one built-in exception is a **tiered semantic palette**: tints/shades of up to three theme colors may be distributed across labeled layers when the layer containers carry the color meaning and no legend is added merely because each layer has its own tint.
...
7. **Final export** — re-export the approved version to all requested formats with escalated / unsandboxed execution. Use `-e` here (PNG/SVG/PDF) so the deliverable stays editable in draw.io; save as `<name>.drawio.png` to signal embedded XML. **For PNG with `-e`, run `python3 <this-skill-dir>/scripts/repair_png.py <name>.drawio.png` immediately after** — draw.io's CLI truncates the IEND chunk in `-e` PNG output (8 bytes missing), producing a corrupt file that vision APIs and strict PNG decoders reject (issue #8). Report file paths.

**If `drawio --version` crashes or prints nothing even after escalated execution (common in restricted macOS sandbox isolation like codex.app):**

- Do not keep retrying CLI invocations inside the sandbox, and do not fall back to running draw.io in-sandbox.
...
Escalation rule (mandatory, before every draw.io CLI call):

- Request escalated / unsandboxed execution before the first and every subsequent draw.io CLI invocation, including `--version`.
- When requesting escalation, fill in the justification/reason with the mandatory rule's template: state that draw.io is an Electron app that may crash or hang inside the sandbox, and that the specific command needs unsandboxed execution. Do not use a vague reason like "I need to run drawio".
- If the binary exists on PATH (or known app path exists) but execution still fails with abnormal exit, empty output, Electron startup failure, display/session error, or likely sandbox restriction, stop retrying and fall back.
- If the binary is missing entirely, do not escalate just to search more aggressively; go to install guidance or fallback.
- If escalation is denied, treat the CLI as unavailable in this environment; never run it inside the sandbox.
...
>
> All commands below are draw.io CLI invocations. In sandboxed agents (Codex included), request escalated execution outside the sandbox before running any of them; never run them inside the sandbox.
...
| draw.io CLI crashes / no output in macOS sandbox isolation | Never run draw.io inside the sandbox; request escalated / unsandboxed execution before every CLI call; if escalation is denied, use browser fallback / XML-only or ask user to run CLI exports in a non-sandboxed host environment |
