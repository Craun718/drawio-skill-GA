...
**Color restraint (required).** Outside black, white, and grays, use soft/muted colors and keep chromatic colors to at most three per diagram unless the user explicitly requests a different palette. Reuse those colors across roles and tiers instead of adding a new hue per node type. A user style preset or an explicit color request overrides this default. For layered architecture, the one built-in exception is a **tiered semantic palette**: one soft hue per labeled layer is acceptable when the layer containers carry the color meaning and no legend is added merely because each layer has its own hue.
